...
class Shot:
    ...
    def __init__(self, gun, position, time_nag, rad, speed, screen, g, parametrs):
        ...
        self.rasst = ((position[0] - gun.coord[0] + (gun.a / 2)) ** 2 +
                      (position[1] - gun.coord[1] - (gun.b / 2)) ** 2) ** 0.5               
...
def napr_rocket(rockets_f, position):
    '''Смена координат ракеты (ракеты, положение мыши в данный момент)'''

    for i in rockets_f:

        if i.flag:
            i.rasst = ((position[0] - i.coord[0]) ** 2 +
                          (position[1] - i.coord[1]) ** 2) ** 0.5
...
def off(rockets_f, coord_prediction_f):
    '''Отключение ракеты в случае достижения ею координат мыши (ракеты, координата мыши в данный момент)'''

    for i in rockets_f:
        R = ((i.coord[0] - coord_prediction_f[0]) ** 2 +
             (i.coord[1] - coord_prediction_f[1]) ** 2) ** 0.5
